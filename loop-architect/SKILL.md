---
name: loop-architect
description: Use this skill to design an execution loop for a coding task before running it. Invoke when the user wants to turn a repetitive, long-running, or recurring task into an autonomous loop (migrations, test/lint burn-downs, bug-hunt sweeps, CI triage, dependency updates), asks to "design a loop", "build a loop for this", "loop this until done", mentions loop engineering, or is manually re-typing the same prompt over and over. Also invoke proactively, without being asked, when the agent notices it is repeating the same shape of change, fix, or prompt across 3 or more items in a session - pause and propose a loop before continuing item by item. The deliverable is a reviewed Loop Card (goal, exit condition, iteration unit, verification, state, stop conditions, human gates) plus a runnable loop prompt. Skip for one-shot tasks, for merely running an already-designed loop on an interval (use the harness's loop runner for that), and for questions about loops in program code (for/while loops, infinite-loop bugs, React render loops).
---
